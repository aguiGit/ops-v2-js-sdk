...
const {getOpsBalance, countFee, inputsOrOutputs, validateTx, broadcastTx, agentDeposistList} = require('./api/util');
...
let fromAddress = "OPSd6HgfzPGhFsZX16hHgneY25YKs6v6LvmX";
...
async function stopAgent(pri, pub, fromAddress, assetsChainId, assetsId, amount, agentHash) {
  const balanceInfo = await getOpsBalance(fromAddress);
  ...
  let tAssemble = await ops.transactionAssemble(newInputs, newOutputss, remark, 9, agentHash);
  ...
  if (transferInfo.fee !== newFee) {
    ...
    tAssemble = await ops.transactionAssemble(newInputs, newOutputs, remark, 9, agentHash);
