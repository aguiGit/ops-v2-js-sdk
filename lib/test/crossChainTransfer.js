...
/*let pri = '9ce21dad67e0f0af2599b41b515a7f7018059418bab892a7b68f283d489abc4b';//tOPSeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG
...
var toAddress = 'tOPSeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG';
...
async function transferTransaction(pri, pub, chainId, transferInfo) {
  ...
  var tAssemble = await ops.transactionAssemble(inputs, outputs, transferInfo.remark, 10); //交易组装
  ...
  if (transferInfo.fee !== newFee) {
    if (chainId === transferInfo.assetsChainId && transferInfo.assetsId === 1) {
      ...
      if (localBalanceInfo.balance < transferInfo.fee) {
        console.log("该账户本链主资产不足够支付手续费！");
        return;
      ops    opstransferInfo.assetsChainId === 2 && transferInfo.assetsId === 1) {
        if (maopstBalanceInfo.balance < transferInfo.amount + newFee) {
          console.log("余额不足");
          return;
        }
        inputs[0].amount = transferInfo.amount + newFee;
   ...
   ops inputs[1].amount = newFee;
        inputs[2].amount = newFee;
      }
    }
    tAssemble = await ops.transactionAssemble(inputs, outputs, transferInfo.remark, 10);
    ctxSign = ops.transactionSignature(pri, tAssemble);
  } else {
    ctxSign = ops.transactionSignature(pri, tAssemble);
  }
  ...
  if (!isMainNet(chainId)) {
    ...
    mainCtxSign = ops.transactionSignature(pri, mainCtx);
    bw.writeBytesWithLength(pubHex);
