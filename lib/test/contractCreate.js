...
module.exports = {
  ...
  createContract: async function createContract(pri, pub, createAddress, assetsChainId, assetsId, contractCreate, remark) {
    ...
    var chainId = contractCreate.chainId;
    var balanceInfo = await getBalance(chainId, assetsChainId, assetsId, createAddress);
    ...
    var tAssemble = await ops.transactionAssemble(inOrOutputs.data.inputs, inOrOutputs.data.outputs, remark, 15, contractCreateTxData);
    var txhex = void 0;
    //获取手续费
    ...
    if (transferInfo.fee !== newFee) {
      ...
      tAssemble = await ops.transactionAssemble(inOrOutputs.data.inputs, inOrOutputs.data.outputs, remark, 15, contractCreateTxData);
