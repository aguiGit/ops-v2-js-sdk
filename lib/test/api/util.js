...
module.exports = {
  ...
  inputsOrOutputsOfContractCall: function inputsOrOutputsOfContractCall(transferInfo, balanceInfo, contractCall, multyAssets) {
    var contractAddress = contractCall.contractAddress;
    var sender = transferInfo.fromAddress;
    var txSizeFee = new BigNumber(transferInfo.fee);
    var newAmount = Number(txSizeFee.plus(transferInfo.amount));
    var newLocked = 0;
    var newNonce = balanceInfo.nonce;
    var newoutputAmount = transferInfo.amount;
    var newLockTime = 0;
    if (balanceInfo.balance < newAmount) {
      return { success: false, data: "Your balance of OPS is not enough." };
    }
    var inputs = [{
      address: transferInfo.fromAddress,
      assetsChainId: transferInfo.assetsChainId,
   ...
   * @returns {Promise<AxiosResponse<any>>}
   */
  getOpsBalance: async function getOpsBalance(address) {
   ...
   * @returns {Promise<T>}
   */
  validateContractCall: async function validateContractCall(sender, value, gasLimit, price, contractAddress, methodName, methodDesc, args, multyAssetArray) {
    return await http.post('/', 'validateContractCall', [sender, value, gasLimit, price, contractAddress, methodName, methodDesc, args, multyAssetArray]).then(function (response) {
      if (response.hasOwnProperty("result")) {
        return { success: true, data: response.result };
      } else {
        return { success: false, data: response.error };
   ...
   * @returns {Promise<T>}
   */
  imputedContractCallGas: async function imputedContractCallGas(sender, value, contractAddress, methodName, methodDesc, args, multyAssetArray) {
    return await http.post('/', 'imputedContractCallGas', [sender, value, contractAddress, methodName, methodDesc, args, multyAssetArray]).then(function (response) {
      if (response.hasOwnProperty("result")) {
        return { success: true, data: response.result };
      } else {
        return { success: false, data: response.error };
