...
exports.encrypt = async function (publicKeyTo, msg) {
    ...
    var Px = await derive(ephemPrivateKey, publicKeyTo);
    var hash = await sha512(Px);
    var encryptionKey = hash.slice(0, 32);
    var macKey = hash.slice(32);
    var ciphertext = await aesCbcEncrypt(iv, encryptionKey, msg);
    var dataToMac = Buffer.concat([iv, ephemPublicKey, ciphertext]);
    var mac = await hmacSha256Sign(macKey, dataToMac);
...
exports.decrypt = async function (privateKey, encrypted) {
    ...
    var macKey = hash.slice(32);
    var dataToMac = Buffer.concat([iv, ephemPublicKey, ciphertext]);
    var macGood = await hmacSha256Verify(macKey, dataToMac, msgMac);
    assert(macGood, "Bad MAC");
    var msg = await aesCbcDecrypt(iv, encryptionKey, ciphertext);
